...
def get_info(steamid):
    ...
    print(f"Уровень игрока: {level_of_player}")
    print(f"За всё время игрок сыграл в {count_of_games} игр")
    print(f"Последние игры: {', '.join(names)}")
    print("Количество друзей: ", count_of_friends)
    print(f"Количество достижений в игре с кодом {appid}: {count_of_achievements}")

get_info(76561199121637283)